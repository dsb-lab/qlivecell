...
model = models.CellposeModel(
    gpu=False,
    pretrained_model="/home/pablo/Desktop/PhD/projects/Data/blastocysts/movies/2h_claire_ERK-KTR_MKATE2/cell_tracking/training_set_expanded_nuc/models/blasto",
)
...
labels_centers, centers_positions, centers_weight, centers_outlines = position3d(
    imgs, Outlines, Masks, centersi, centersj, labels
)
...
new_outline = np.asarray(
    [
        list(a)
        for a in zip(
            np.rint(linebuilder.xs).astype(np.int64),
            np.rint(linebuilder.ys).astype(np.int64),
        )
    ]
)
