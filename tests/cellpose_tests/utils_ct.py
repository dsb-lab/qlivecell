...
def cell_segmentation_outlines(imgs, cellposemodel=None, trainedmodel=False):
    ...
    for z in range(slices):
        ...
        if trainedmodel:
            ...
            masks, flows, styles, diam = cellposemodel.eval(img, channels=[0,0], flow_threshold=0.4)
...
def compute_distances_with_pre_post_z(imgs, centersi, centersj, distth=1.5, xyresolution=0.2767553):
    slices = imgs.shape[0]
    distances_idx = []
    distances_val = []
...
def detect_cell_barriers(imgs, labels, Masks, neigh=1,relative=True, fullmat=True, overlap_th=0.15): # Need to consider changing also masks outlines etc
    intensities = nuclear_intensity_cell_z(imgs, labels, Masks)
    Zoverlap_conv = compute_overlap_measure(labels, Masks, neigh=neigh,relative=relative, fullmat=fullmat)
    Zlabel_l, Zlabel_z = label_per_z(labels)
...
def separate_concatenated_cells(imgs, labels, Outlines, Masks, neigh=1,relative=True, fullmat=True, overlap_th=0.15):
    Zlabel_l, Zlabel_z = label_per_z(labels)
    cellbarriers = detect_cell_barriers(imgs, labels, Masks, neigh=neigh, relative=relative, fullmat=fullmat, overlap_th=overlap_th)
    zids_remove = []
    labs_remove = []
        ...
        for i, z in enumerate(Zlabel_z[c]):
            ...
            for j, zz in enumerate(zvec):
                if zz!=z:
                    ...
                    Zoverlap[c][i,j]=compute_overlap(mask_curr, mask_other, relative=relative)
    return Zoverlap
...
def compute_overlap_measure(labels, Masks, neigh=1,relative=True, fullmat=True):
    Zoverlaps = compute_planes_overlap(labels, Masks, relative=relative, fullmat=fullmat)
    Zoverlaps_conv = []
    for c, Zoverlap in enumerate(Zoverlaps):
        ...
        for z in range(Zoverlap.shape[0]):
            val = 0.0
            n   = 0
            for i in range(np.maximum(z-neigh, 0), np.minimum(z+neigh+1, Zoverlap.shape[0])):
                if i!=z:
                    val+=Zoverlap[z, i]
                    n+=1
            if n == 0:
