...
class EmbryoSegmentation:
    def __init__(
        ...
        if trange is None:
            self.trange = range(self.times)
        else:
            self.trange = trange
        if zrange is None:
            self.zrange = range(self.slices)
        else:
            self.zrange = zrange
        ...
        if type(binths) == list:
            if len(binths) == 2:
                ...
                if apply_biths_to_zrange_only:
                    self.binths[self.zrange] = np.linspace(
                        binths[0], binths[1], num=len(self.zrange)
                    )
                else:
                    self.binths = np.linspace(binths[0], binths[1], num=self.slices)
            else:
                self.binths = binths
        else:
            self.binths = [binths for i in range(self.slices)]
        ...
        for tid, t in enumerate(self.trange):
            ...
            if self._threads is None:
                ...
                for zid, z in enumerate(self.zrange):
                    result = compute_emb_masks_z(
                        IMGS[t][z], z, tid, zid, self.binths[z], seg_embryo_params
                    )
    ...
    def plot_segmentation(
        self, ts, zs, plot_background=True, extra_IMGS=None, extra_title=""
    ):
