...
from embdevtools import get_file_embcode, read_img_with_resolution, embryoregistration


import os

home = os.path.expanduser('~')
path_parent = home+'/Desktop/PhD/projects/Data/blastocysts/Lana/16cell_blastocyst/cropped8bit/'
...
path_registered, path_output, path_movies_reg = embryoregistration.generate_fijiyama_file_system(path_parent, 'movies_registered', embcode)

# Save registration stacks into input folder
embryoregistration.generate_fijiyama_stacks(path_registered, IMGS_corrected, xyres, zres, file_format="t%d.tif")
...
path_trans_emb_global, path_trans_emb_steps = embryoregistration.create_transformations_folders(path_movies_reg, embcode, trans_folder="transformations")

# Move transformations
embryoregistration.move_transformation(path_output, path_trans_emb_global, path_trans_emb_steps)
...
path_movies_reg_embcode = embryoregistration.create_dir(path_movies_reg, embcode, return_path=True, rem=True)
...
for ch, IMGS_ch in enumerate(IMGS_chs):
    ...
    path_movies_reg_embcode_ch = embryoregistration.create_dir(path_movies_reg_embcode,'%d' %ch, return_path=True, rem=True)
    path_movies_reg_embcode_ch_reg = embryoregistration.create_dir(path_movies_reg_embcode,'registered_%d' %ch, return_path=True, rem=True)

    embryoregistration.generate_fijiyama_stacks(path_movies_reg_embcode_ch, IMGS_ch, xyres, zres, file_format="t%d.tif", rem=True)

    # Define where you have the beanshell class to be called from beanshell
    pth_beanshell = "/opt/Fiji.app/beanshell/bsh-2.0b4.jar"
    text_to_write =  "\n".join([pth_beanshell, path_trans_emb_global, path_movies_reg_embcode_ch, embryoregistration.correct_path(path_movies_reg_embcode_ch_reg)])
    ...
    pth_beanshell_script = embryoregistration.correct_path('/home/pablo/Desktop/PhD/projects/embdevtools/pyjiyama/')+'utils/apply_transformation.bsh'
    import subprocess
    subprocess.run(['/opt/Fiji.app/ImageJ-linux64', '--headless' , pth_beanshell_script])#, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    for _t, tfile in enumerate(tfiles):
        ...
        print(t)
        IMG_t, xyres, zres = read_img_with_resolution(embryoregistration.correct_path(path_movies_reg_embcode_ch_reg)+tfile, channel=None)
