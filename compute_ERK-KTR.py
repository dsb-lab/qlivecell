...
if __name__ == '__main__':
    ...
    # EmbSeg = EmbryoSegmentation(IMGS_ERK, ksize=5, ksigma=3, binths=10, checkerboard_size=6, num_inter=100, smoothing=5, trange=range(1), mp_threads=10)
    # EmbSeg()
    ...
    start = time.time()
    erkktr = ERKKTR(IMGS_ERK, innerpad=1, outterpad=2, donut_width=4, min_outline_length=100, mp_threads=10)
    erkktr.create_donuts(cells, EmbSeg)
    end = time.time()
    print(end - start)
    
    start = time.time()
    erkktr.correct_donut_embryo_overlap(cells, EmbSeg)
    end = time.time()
    print(end - start)

    erkktr.plot_donuts(cells, IMGS_SEG, IMGS_ERK, 0, 15, plot_nuclei=False, plot_outlines=True, plot_donut=True, EmbSeg=EmbSeg)
